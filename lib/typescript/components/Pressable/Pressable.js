...
export default function Pressable(props) {
    const { testOnly_pressed, hitSlop, pressRetentionOffset, delayHoverIn, onHoverIn, delayHoverOut, onHoverOut, delayLongPress, unstable_pressDelay, onPress, onPressIn, onPressOut, onLongPress, style, children, android_disableSound, android_ripple, disabled, ...remainingProps } = props;
    ...
    const longPressMinDuration = (delayLongPress ?? DEFAULT_LONG_PRESS_DURATION) +
    ...
    const pressAndTouchGesture = useMemo(() => Gesture.LongPress()
        ...
        pressableRef.current?.measure((_x, _y, width, height) => {
            if (!isTouchWithinInset({
                width,
                height,
            }, normalizedHitSlop, event.changedTouches.at(-1)) ||
    ...
    return (<View {...remainingProps} style={outerStyles}>
      <GestureDetector gesture={gesture}>
        <NativeButton ref={pressableRef} hitSlop={appliedHitSlop} enabled={isPressableEnabled} touchSoundDisabled={android_disableSound ?? undefined} rippleColor={processColor(android_ripple?.color ?? defaultRippleColor)} rippleRadius={android_ripple?.radius ?? undefined} style={[
