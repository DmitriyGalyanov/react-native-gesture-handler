...
export default class PointerTracker {
    ...
    getVelocity(pointerId) {
        return {
            x: this.trackedPointers.get(pointerId)?.velocityX,
            y: this.trackedPointers.get(pointerId)?.velocityY,
        };
    }
    getLastAbsoluteCoords(pointerId) {
        if (pointerId !== undefined) {
            return {
                x: this.trackedPointers.get(pointerId)?.abosoluteCoords.x,
                y: this.trackedPointers.get(pointerId)?.abosoluteCoords.y,
            };
        }
        else {
            return {
    ...
    getLastRelativeCoords(pointerId) {
        if (pointerId !== undefined) {
            return {
                x: this.trackedPointers.get(pointerId)?.relativeCoords.x,
                y: this.trackedPointers.get(pointerId)?.relativeCoords.y,
            };
        }
        else {
            return {
